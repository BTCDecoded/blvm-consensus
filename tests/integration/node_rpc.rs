...
impl NodeRpcClient {
    ...
    pub async fn call(
        ...
        let response = request.json(&rpc_request).send().await?;
    ...
    pub async fn test_mempool_accept(
        &self,
        tx_hex: &str,
    ) -> Result<MempoolAcceptResult, Box<dyn std::error::Error>> {
    ...
    pub async fn get_block(
        ...
        self.call("getblock", json!([hash, verbosity.unwrap_or(1)]))
            .await
    }
}
...
pub async fn compare_transaction_validation_via_rpc(
    ...
    let local_result = blvm_consensus::transaction::check_transaction(tx)?;
...
mod tests {
    ...
    async fn test_transaction_validation_comparison() {
        ...
        if let Ok(result) = comparison {
            if !result.divergence {
                ...
                panic!(
                    "Divergence between RPC and direct validation: {:?}",
    ...
    fn test_mempool_accept_result_parsing() {
        ...
        assert_eq!(
            ...
            Some("bad-txns-inputs-missing".to_string())
        );
    }
}
